...
export fn Java_com_jossephus_chuchu_service_terminal_GhosttyBridge_nativeSetCellSize(env: *c.JNIEnv, thiz: c.jobject, handle: c.jlong, cell_width_px: c.jfloat, cell_height_px: c.jfloat) callconv(.c) void {
    _ = env;
    _ = thiz;
    chuchu_set_cell_size(handle, cell_width_px, cell_height_px);
}
...
export fn chuchu_resize(handle: c.jlong, cols: c.jint, rows: c.jint, cell_width: c.jint, cell_height: c.jint) callconv(.c) void {
    ...
    // Keep float cell dimensions in sync when only int values are provided.
    if (terminal.cell_width_px <= 0) terminal.cell_width_px = @floatFromInt(terminal.cell_width);
...
export fn chuchu_set_cell_size(handle: c.jlong, cell_width_px: c.jfloat, cell_height_px: c.jfloat) callconv(.c) void {
    const terminal = chuchuFromHandle(handle) orelse return;
    if (cell_width_px > 0) terminal.cell_width_px = cell_width_px;
    if (cell_height_px > 0) terminal.cell_height_px = cell_height_px;
}
